...
class Run_Main():
    ...
    def loop_run(self):
        while True:
            ...
            if grid_buy_price >= cur_market_price:  # 当前价格低于设定的网格买入价

                if future_step != 0:  # 说明期货有仓位 则卖出 仓位-1
                    profit_usdt = round(
                        (grid_buy_price / (1 - self.profitRatio / 100) - grid_buy_price) * runbet.get_future_quantity(
                            False), 2)  # 计算 本次盈利u数(买卖价差*数量)
                    future_res = msg.buy_limit_future_msg(self.coinType, runbet.get_future_quantity(False),
                                                          grid_buy_price, profit_usdt)  # 期货卖出
                ...
                if spot_step != 0:  # 说明现货有仓位 则卖出 仓位-1
                    profit_usdt = round(
                        (grid_sell_price / (1 + self.profitRatio / 100) - grid_sell_price) * runbet.get_future_quantity(
                            False), 2)  # 计算 本次盈利u数(买卖价差*数量)
                    spot_res = msg.sell_limit_msg(self.coinType, runbet.get_spot_quantity(False), grid_sell_price,
                                                  profit_usdt)  # 期货卖出开多
